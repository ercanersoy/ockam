...
TransportError PosixTcpReceiveBlocking(Connection *connection, void *buffer, uint16_t size, uint16_t *bytesReceived) {
  ...
  while((kMoreData == p_transmission->status) && (p_transmission->buffer_remaining > 0)) {
    ...
    recv_status = recv(p_tcp->posixSocket.socket,
        p_transmission->buffer+bytes_read,
        bytes_to_read, 0);
    if(-1 == recv_status) {
      status = kReceive;
    ...
    if(p_transmission->bytes_transmitted < p_transmission->transmit_length) p_transmission->status = kMoreData;
    else p_transmission->status = kErrorNone;
  }
